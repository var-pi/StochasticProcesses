...
\item[Negative control] Set $\texttt{jitter}=0$ and factor the \emph{same} Brownian-motion $\Sigma$ used in the main check: its $t=0$ row is identically zero ($R(0,s)=0$), so $\Sigma$ is exactly singular and \texttt{cholesky} throws \texttt{PosDefException}. The throw disappears once $\varepsilon\gtrsim10^{-10}$ --- so the nugget is shown to be load-bearing for the very $\Sigma$ used above, not for a separate pathological matrix. (Reusing the main-check $\Sigma$ is the more honest control: the exponential kernel is well-conditioned at ordinary grid resolutions --- $\operatorname{cond}\approx6\times10^4$ at 200 points --- and would \emph{not} throw at $\texttt{jitter}=0$.)
...
\item[Check] (a) \emph{Route equivalence:} Cholesky, KL, and circulant samples of the OU process yield the same empirical covariance, judged against a \emph{split-half null} --- the same statistic computed between two halves of a \emph{single} route's samples, which is by construction a draw from the ``same law, different samples'' distribution and so calibrates the gate without a variance formula. (b) \emph{Distributional identities}, in two steps: Gaussianity of the transformed process is established \emph{by construction} (a deterministic linear time-change of a Gaussian process is Gaussian, laws being closed under linear maps), after which App.~B.5 applies as a \emph{theorem} and matching the \emph{full} covariance over the grid is equality in law --- e.g.\ $c^{-1/2}W(ct) \overset{d}{=} W(t)$; then, as a check that does \emph{not} lean on Gaussianity, Cram\'er--Wold projections: a handful of fixed linear functionals $a^\top X$ of the sample paths, each one-dimensional projection KS-tested against its $N(0,a^\top\Sigma a)$ target. Bridge endpoints pinned exactly at $t=0,1$ via the covariance/Cholesky route (the time-change formula cannot be evaluated at $t=1$). Additionally the KL coefficients: project onto Unit~2's eigenfunctions, verify the empirical correlation matrix of the $\xi_k$ is $\approx I$ and KS each $\xi_k$ against $N(0,\lambda_k)$ --- uncorrelated \emph{and} jointly Gaussian $\Rightarrow$ independent, which is the KL expansion's actual content. (c) \emph{The cross-check} (below).
\item[Analytic target] Kernels $\min(t,s)$, $\min(t,s)-ts$, $\tfrac{D}{\alpha}e^{-\alpha|\tau|}$. \textbf{The cross-check target is asymptotic, not exact.} On $[0,T]$ the OU KL eigenvalues do \emph{not} equal the sampled spectral density; the coincidence $\lambda_k=\hat R(k)$ is torus-only. What holds on $[0,T]$ is the large-$T$ Toeplitz/Grenander--Szeg\H{o} limit $\lambda_k \to \hat R(\omega_k)$, $\omega_k = k\pi/T$, where $\hat R(\omega)=\int_{-\infty}^{\infty}e^{-i\omega\tau}R(\tau)\,d\tau=\tfrac{2D}{\alpha^2+\omega^2}=2\pi\,S(\omega)$ is the \emph{un-normalized} operator symbol: \texttt{nystrom\_eigen} diagonalizes the bare integral operator $\int R(t,s)e(s)\,ds$ (no $1/2\pi$), so its eigenvalues converge to $\hat R$, \emph{not} to the Unit-1 density $S(\omega)=\tfrac{D}{\pi(\alpha^2+\omega^2)}$ (which is $2\pi$ smaller). This is the interval counterpart of the exact torus coincidence $\lambda_k=\hat R(k)$ above. \textbf{And the convergence is distributional, not uniform in $k$:} Szeg\H{o} controls linear statistics of the spectrum, not each individual eigenvalue, and is weakest exactly at the spectrum's edges --- so there is \emph{no} defensible per-eigenvalue target, the comparison is restricted to the resolved bulk (reusing Unit~2's resolved-range criterion), and no theoretical decay exponent for the gap is claimed. \extnote{this is the Toeplitz asymptotic flagged in \S1.2, not a Pavliotis result; it is the reason the cross-check is stated as a convergence rather than an equality, and the reason it is gated on the bulk rather than on a $\max_k$ over the whole spectrum.}
\item[Pass/fail] \emph{Route equivalence --- gate against a split-half null, not an SE.} The statistic $\|\widehat\Sigma_A-\widehat\Sigma_B\|_F$ is a norm over $n_{\text{grid}}^2$ entries: it does \emph{not} hover near zero but concentrates about a strictly positive value, $\mathbb{E}\|\widehat\Sigma_A-\widehat\Sigma_B\|_F^2 = 2\sigma_1^2$ with $\sigma_1^2 = \mathbb{E}\|\widehat\Sigma-\Sigma\|_F^2 = \big((\operatorname{tr}\Sigma)^2+\|\Sigma\|_F^2\big)/N$, so a zero-centred ``within $3\,$SE'' predicate is gating against the wrong null and passes routes that disagree by a factor of two. Instead: split one route's $N$ samples into halves, compute $\|\widehat\Sigma_{A,1}-\widehat\Sigma_{A,2}\|_F$ over several splits to obtain the null band empirically, and require each cross-route statistic to land inside it after the $\sqrt2$ rescale for the halved sample size. Self-calibrating and correctly powered; the closed form above is retained only as a sanity check that the measured band sits where theory says. \emph{Distributional identity:} full empirical covariance of $c^{-1/2}W(ct)$ matches $\min(t,s)$ within the same split-half null, \emph{and} every Cram\'er--Wold projection's KS statistic lies below its finite-sample critical value at the recorded seed. \emph{Cross-check:} with $\lambda_k$ from \texttt{nystrom\_eigen} and $\hat R(\omega_k)$ \emph{analytic} (not Welch-estimated), the bulk gap $g(T)=\max_{k\in\text{bulk}}|\lambda_k - \hat R(\omega_k)|$ is deterministic given the quadrature; fit $\log g$ vs.\ $\log T$ across a ladder of increasing $T$ and gate the fitted slope below a fixed negative threshold, reporting the exponent without claiming it against a theoretical value (none is defensible --- see Analytic target). Per the rate-check convention of \emph{Reproducibility and Testing Convention} below this is a \emph{deterministic} slope, so the margin is a fixed absolute one and \emph{not} a multiple of the regression SE; monotonicity is deliberately not required, since neither Szeg\H{o} nor the quadrature guarantees it at finite $T$.
...
\item[Artifact] A three-panel portrait per process (sample paths / covariance heat-map / eigenvalue decay); a route-equivalence panel (three empirical covariances of OU, with the three pairwise distances plotted \emph{against the split-half null band}, so the reader sees the gate's calibration and not merely a number); a Cram\'er--Wold panel (per-projection empirical CDF against its Gaussian target, with the KS statistic annotated); the cross-check plot, $\log g(T)$ vs.\ $\log T$ on log--log axes with the fitted slope shown --- and the Welch estimate of $S$ overlaid on the analytic $S$ at the largest $T$ as a \emph{visual} reconciliation of the two diagonalizations (this overlay is the pedagogical point of the unit; it is not what the gate is computed from).
\item[README] Concept: each process is just a choice of kernel. Why this check: distributional identities certify the \emph{law}, not merely a moment --- and the README must spell out the licensing chain rather than assert the conclusion, because the chain is the content: the time-changed process is Gaussian \emph{by construction} (a deterministic linear map of a Gaussian process), which is a proof and not something the samples are asked to establish; App.~B.5 then upgrades a match of the \emph{full} covariance into equality in law; and the Cram\'er--Wold projections supply the evidence that would catch a non-Gaussian impostor carrying the right covariance, which is precisely the evidence App.~B.5 alone cannot give. The cross-check forces the two diagonalizations to reconcile as $T\to\infty$. What it proves: the field's vocabulary is correctly instantiated, the sampling routes agree, and the Fourier and KL vertices are made to speak to each other directly --- the payoff of \S1.5. Its distinct idea is reconciliation; the sampling is inherited.
\item[Missteps] (i) \emph{Expecting exact, or uniform, $\lambda_k=\hat R(\omega_k)$ on $[0,T]$} --- it is asymptotic \emph{and} distributional; gate on the bulk gap shrinking, never on a single-$T$ match, and never on a $\max_k$ over the whole spectrum, where the edge eigenvalues Szeg\H{o} does not control will dominate the statistic and can stall its decay entirely. (ii) \emph{Frequency grid and normalization for the cross-check} --- use $\omega_k=k\pi/T$, and compare $\lambda_k$ against the \emph{un-normalized} symbol $\hat R(\omega)=2\pi\,S(\omega)=\tfrac{2D}{\alpha^2+\omega^2}$, \emph{not} the Unit-1 density $S$: \texttt{nystrom\_eigen} carries no $1/2\pi$, so comparing against $S$ is off by a constant factor of $2\pi$ and the gap plateaus instead of shrinking. (iii) \emph{Distributional vs moment identity} --- equality of mean and covariance implies equality in law only for jointly Gaussian objects \pav{App.~B.5}, so the Gaussianity must be \emph{argued} (here: by construction, from linearity of the time change) and never inferred from the samples that are then tested; given that argument, a match of the \emph{full} covariance \emph{is} a distributional identity and should be claimed as one --- the error to avoid is the opposite one, claiming the law from a \emph{single} matched moment with the Gaussianity left implicit. (iv) \emph{Gating $\|\widehat\Sigma_A-\widehat\Sigma_B\|_F$ against zero} --- it is a norm over many entries, so it concentrates around $\sqrt2\,\sigma_1>0$ rather than fluctuating about zero; a tolerance phrased as a multiple of a single-route SE is measuring against a null the statistic never visits and will pass badly disagreeing routes. Guard: calibrate with the split-half null, and confirm the observed band matches $\sqrt2\,\sigma_1$ before trusting it.
...
\item[Effort] $\approx 8$ h (the sampling leverages Units 0, 1, and 2 and is almost entirely reuse; the added hour is the Cram\'er--Wold projections, the split-half calibration, and \texttt{gof.jl} with its test).
\item[Prerequisite] For jointly Gaussian objects, equality of mean and covariance implies equality in law \pav{App.~B.5} --- the recurring fact that uncorrelated $+$ jointly Gaussian $\Rightarrow$ independent is used here to validate KL coefficients. New content: verifying a \emph{distributional} identity (not just a moment) from finite Monte Carlo samples, which is done in two moves --- App.~B.5 applied to a Gaussianity that is proved rather than tested, and then the Cram\'er--Wold device, that equality in law of random vectors is equivalent to equality in law of every one-dimensional projection $a^\top X$, which reduces a multivariate distributional claim to a family of scalar KS tests. \extnote{the Cram\'er--Wold theorem, and the Kolmogorov--Smirnov statistic and its finite-sample critical values; standard, external to Pavliotis.}
\end{unitmeta}
...
\texttt{gaussianprocess.jl} & \texttt{empirical\_cov(paths)} & per-route covariance; also per half, for the split-half null \\
\texttt{gof.jl} \textbf{(new)} & \texttt{ks\_statistic(samples, cdf)} & KS distance to a target CDF; Cram\'er--Wold projections \\
\texttt{kl.jl} & \texttt{nystrom\_eigen(R, nodes, weights)} & $\lambda_k$ for the cross-check \\
\texttt{spectral.jl} & \texttt{welch\_psd(x, dt; ...)} & the $S$ overlay in the artifact only --- \emph{not} the gate \\
\end{contracttable}
\noindent\textbf{One new \texttt{src/} module, \texttt{gof.jl}} --- a small, tested goodness-of-fit utility (\texttt{ks\_statistic}), shared with Unit~5. It is deliberately \emph{not} an operation on $\mathcal{C}$; it lives in \texttt{src/} rather than in either experiment because two units need it and because a hand-computed deterministic test is exactly the kind of guard the \texttt{src/}\slash\texttt{test/} tier exists to provide. Everything else in this unit is reuse. \textbf{\texttt{experiments/03\_process\_zoo/run.jl}} produces: (i) three per-process portraits; (ii) the OU route-equivalence panel, with the three pairwise distances against the split-half null band; (iii) the BM invariance identities, the Cram\'er--Wold per-projection KS table, the KL-coefficient correlation/normality check, and the pinned bridge endpoints; (iv) the deterministic cross-check ladder $g(T)=\max_{k\in\text{bulk}}|\lambda_k-\hat R(\omega_k)|$ vs.\ $T$ on log--log axes with its fitted slope, plus the one-sided Welch-vs-analytic overlay (against $2S(\omega)$); (v) the wrong-exponent negative-control panel.
...
\texttt{gof.jl} & \texttt{ks\_statistic(samples, cdf)} & KS distance of $W^n_1$ to $N(0,1)$; reused from Unit~3 \\
\texttt{gaussianprocess.jl} & \texttt{empirical\_cov(paths)} & two-time covariance $\to\min(s,t)$ \\
\end{contracttable}
\noindent\textbf{No new \texttt{src/} code} --- the \emph{process} builders live in the experiment, weak convergence of laws not being an operation on $\mathcal{C}$; the one library call is \texttt{gof.jl}'s \texttt{ks\_statistic}, introduced by Unit~3 and reused here rather than duplicated. \textbf{\texttt{experiments/05\_bm\_scaling\_limit/run.jl}} produces: (i) overlaid rescaled paths at increasing $n$; (ii) marginal histogram/QQ vs.\ $N(0,1)$; (iii) $(n,\text{KS})$ log--log, one line per increment law; (iv) empirical vs.\ $\min(s,t)$; (v) running-max across laws; (vi) the infinite-variance control.
...
|   |-- gof.jl                      # KS statistic (Cramer-Wold checks, Donsker
|   |                               #   rates); not an operation on C -- see below
...
\begin{itemize}[leftmargin=1.4cm]
  ...
  \begin{itemize}[leftmargin=1.0cm]
    ...
    \item \emph{Deterministic slopes} (the KL eigenvalue-decay slope of Unit~2; the Toeplitz bulk-gap slope $g(T)$ of Unit~3): here the regression residuals are model misspecification --- the asymptotic $k^{-2}$ plus the discretization noise floor for Unit~2, the Szeg\H{o} asymptotic plus the quadrature error for Unit~3 --- \emph{not} sampling noise, so a regression standard error is not a sampling standard error and the SE-based gate is not statistically meaningful. Fit only within the resolved window (above the noise floor, or within the resolved spectral bulk) and gate with a \emph{fixed absolute margin}: for Unit~2 against the theoretical exponent $-2$; for Unit~3, where no defensible theoretical exponent exists, against a fixed negative threshold, with the fitted value reported rather than claimed. The distinction that decides which bullet applies is not whether the \emph{process} is random but whether the \emph{regression residuals} are sampling noise --- Unit~3's gap becomes deterministic precisely because it is computed against the analytic $S$ rather than a Welch estimate.
  \end{itemize}
...
\subsection*{\texttt{src/gof.jl}}
\begin{lstlisting}
module GOF

export ks_statistic

"Two-sided Kolmogorov-Smirnov distance sup_x |F_n(x) - F(x)| between the
 empirical CDF of `samples` and a target CDF `cdf` (a callable). The sup is
 attained at a sample point, so it suffices to check both one-sided gaps at
 each order statistic: F_n jumps from (i-1)/n to i/n at the i-th one.

 Deliberately NOT an operation on the covariance operator -- this is the one
 module that sits outside the src/ organizing principle. It is here, rather
 than duplicated in two run.jl files, because Unit 3 needs it for the
 Cramer-Wold projections and Unit 5 for the Donsker marginal rates, and
 because src/ is the tier that gets a deterministic test."
...
@testset "KS statistic, hand-computed" begin
    # Uniform(0,1) target, so F(x) = x and the gaps are computable by hand.
    # samples 0.1, 0.5, 0.9 (n = 3); at the i-th order statistic F_n jumps
    # from (i-1)/3 to i/3, so both one-sided gaps must be checked:
    #   i=1, x=0.1:  1/3 - 0.1 = 7/30   |   0.1 - 0   = 3/30
    #   i=2, x=0.5:  2/3 - 0.5 = 5/30   |   0.5 - 1/3 = 5/30
    #   i=3, x=0.9:  3/3 - 0.9 = 3/30   |   0.9 - 2/3 = 7/30
    # sup = 7/30. This symmetric case does NOT discriminate a one-sided
    # implementation (either gap alone still returns 7/30) -- the two skewed
    # cases below are the ones that bite, and they must BOTH be kept.
    @test isapprox(ks_statistic([0.1, 0.5, 0.9], x -> x), 7 / 30; atol = 1e-12)
    # unsorted input must agree: guards a missing sort.
    @test isapprox(ks_statistic([0.9, 0.1, 0.5], x -> x), 7 / 30; atol = 1e-12)
    # bunched left: the UPPER gap attains the sup; a D-minus-only impl gets 0.1.
    #   i=1, x=0.1: 1/2 - 0.1 = 0.4  | 0.1 - 0   = 0.1
    #   i=2, x=0.2: 2/2 - 0.2 = 0.8  | 0.2 - 1/2 < 0
    @test isapprox(ks_statistic([0.1, 0.2], x -> x), 0.8; atol = 1e-12)
    # bunched right (the mirror): the LOWER gap attains it; a D-plus-only
    # impl gets 0.1. Together these two pin both sides of the jump.
    #   i=1, x=0.8: 1/2 - 0.8 < 0    | 0.8 - 0   = 0.8
    #   i=2, x=0.9: 2/2 - 0.9 = 0.1  | 0.9 - 1/2 = 0.4
