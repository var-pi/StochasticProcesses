...
\item[Missteps] (i) \emph{Expecting exact, or uniform, $\lambda_k=S(\omega_k)$ on $[0,T]$} --- it is asymptotic \emph{and} distributional; gate on the bulk gap shrinking, never on a single-$T$ match, and never on a $\max_k$ over the whole spectrum, where the edge eigenvalues Szeg\H{o} does not control will dominate the statistic and can stall its decay entirely. (ii) \emph{Frequency grid for the cross-check} --- use $\omega_k=k\pi/T$ and the same $1/2\pi$ convention as Unit~1, or the comparison is off by a constant. (iii) \emph{Distributional vs moment identity} --- equality of mean and covariance implies equality in law only for jointly Gaussian objects \pav{App.~B.5}, so the Gaussianity must be \emph{argued} (here: by construction, from linearity of the time change) and never inferred from the samples that are then tested; given that argument, a match of the \emph{full} covariance \emph{is} a distributional identity and should be claimed as one --- the error to avoid is the opposite one, claiming the law from a \emph{single} matched moment with the Gaussianity left implicit. (iv) \emph{Gating $\|\widehat\Sigma_A-\widehat\Sigma_B\|_F$ against zero} --- it is a norm over many entries, so it concentrates around $\sqrt2\,\sigma_1>0$ rather than fluctuating about zero; a tolerance phrased as a multiple of a single-route SE is measuring against a null the statistic never visits and will pass badly disagreeing routes. Guard: calibrate with the split-half null, and confirm the observed band matches $\sqrt2\,\sigma_1$ before trusting it.
