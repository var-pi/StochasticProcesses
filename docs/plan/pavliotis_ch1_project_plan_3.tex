...
nonzero --- for a symmetric increment the leading $n^{-1/2}$ Edgeworth term vanishes, leaving the
$O(n^{-1})$ term. So the target is $\propto n^{-1/2}$ for an \emph{exponential} (skewed) increment and
$\propto n^{-1}$ for \emph{Rademacher} and \emph{uniform} (symmetric) ones. The headline artifact is
