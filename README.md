...
| **3** — [`process zoo`](experiments/03_process_zoo/) | Brownian motion, Brownian bridge, stationary OU as a catalogue of kernels; three sampling routes reconciled; distributional identities from finite samples; Toeplitz/Szegő cross-check | The three sampling routes (Cholesky, KL, circulant) agree within a split-half null; `c^{-1/2}W(ct) =d W(t)` holds as a distributional identity (Gaussian-by-construction + Cramér–Wold); and the OU eigenvalues converge to the analytic symbol `2π·S(ω)`, the fitted log–log gap slope landing at **−0.89**. |
| **4** — [`ergodicity`](experiments/04_ergodicity/) | Stationary OU time-average estimators; the finite-T variance identity (Lemma 1.17) and the Green–Kubo coefficient `D*=D/α²`; non-integrable-`C` falsifier | The time-average variance decays as `T⁻¹` (fitted log–log slope **−0.993**, within 2.5× its sub-ensemble SE) with constant `2D*` (median `T·Var` **0.495** vs 0.500 — pinning the `α²`, not `α`, dependence); the integrated MSD grows as `2D*t`; and a **non-integrable** correlation `C(u)=(1+u)^{-½}` breaks the rate (slope **−0.41**, tracking the exact curve, far from −1) — Prop. 1.16's `L¹` hypothesis shown load-bearing. |
| **5** — [`random walk → BM`](experiments/05_bm_scaling_limit/) | Brownian motion as the weak (Donsker) limit of a rescaled random walk `W_n(t)=S_{⌊nt⌋}/√n`; three increment laws; marginal-convergence *rate*, running-max functional, infinite-variance falsifier | The rescaled-walk covariance lands on `min(s,t)`; the marginal `S_n/√n → N(0,1)` splits into **three** rates — skewed **and** lattice laws both at `n^{-½}` (fitted **−0.502**, **−0.497**), smooth-symmetric a full power faster at `n^{-1}` (exact Irwin–Hall **−1.009**); the running maximum converges to the half-normal `|N(0,1)|` (KS ≤ 0.024, wrong-target Φ pinned at 0.500); and an **infinite-variance** symmetric-Pareto (`γ=1.5`) increment breaks Donsker — variance *grows* (slope **+0.42**) instead of settling — the finite-variance hypothesis shown load-bearing. |
